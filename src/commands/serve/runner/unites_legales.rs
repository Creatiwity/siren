...
async fn get_unite_legale_by_siren(
    ...
    let unite_legale = match models::unite_legale::get(&mut connection, &siren).await {
        Ok(u) => u,
    ...
    let etablissements = models::etablissement::get_with_siren(&mut connection, &siren).await?;
    let etablissement_siege =
        models::etablissement::get_siege_with_siren(&mut connection, &unite_legale.siren).await?;
